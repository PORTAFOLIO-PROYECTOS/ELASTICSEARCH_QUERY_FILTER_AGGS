...
module.exports = class Filtros {
    ...
    filtro() {
        ...
        let filtroOrigenDistinctElastic = utils.distinctInArray(this.params.filtroOrigen, 'ElasticsearchCampo'); // se obtiene los campos por el cual se hizo el agregate en elastic
        let filtroOrigen = this.params.filtroOrigen;
        let filtrosElasticsearch = this.params.elasticsearchData.aggregations;
        let resultado = [];
        let dataElastic = [];

        filtroOrigenDistinctElastic.forEach(item => {
            let data = filtrosElasticsearch[item.ElasticsearchCampo].buckets;
...
console.log(dataElastic);
        for (let i = 0; i < filtroOrigenDistinct.length; i++) {
            ...
            for (let j = 0; j < filtroSeccionOrigen.length; j++) { // se recorre los filtros de la sección
                ...
                let filtro = dataElastic.find(x => x.key === element.FiltroNombre); // se seleciona el filtro en la data de elastic 
